...
While working under a NIST PSCR grant, I prototyped a tracked device for our research team's simulations. Such devices could be used to track AR displays in VR. Based on discussions with first responders, it was determined that anchoring the virtual UI to an armband would be useful. The location is familiar for those who wear a wristwatch and can be easily moved out of the way when not needed. To not clutter one’s vision. In a real traffic stop scenario, a wristband with buttons might be worn to enable and disable the device.
